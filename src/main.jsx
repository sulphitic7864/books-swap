...
const img = (n) => new URL(`./assets/${n}`, import.meta.url).href;
const genres = [
  "Fiction",
  "Mystery",
  "Science",
...
const avails = ["Available", "Reserved", "Exchanged"];
function Layout() {
    ...
    <>
      <header className="top">
        ...
        <nav className={open ? "show" : ""} onClick={() => setOpen(false)}>
          {["Home:/", "Browse:/books", "Community:/community"].map((x) => {
            let [a, b] = x.split(":");
            return (
              <NavLink key={b} to={b}>
                {a}
          ...
          {user ? (
            ...
            <NavLink className="btn small" to="/login">
              Sign in
            </NavLink>
          )}
        </nav>
...
function Owner({ children }) {
  ...
  const { id } = useParams();
  return user && state.books.find((b) => b.id === id)?.ownerId === user.id ? (
    children
  ) : (
  ...
  return (
    <section className="auth">
      <form className="panel" onSubmit={submit}>
        <h1>
          {mode === "register"
            ? "Create your BookSwap account"
            : "Welcome back"}
        </h1>
        <p className="muted">
          {mode === "login"
            ? "Use a supplied demo account or create a local profile."
            : "Join with a neighborhood so exchanges stay practical."}
        </p>
...
function Catalog() {
  ...
  let locs = [...new Set(state.books.map((b) => b.location))];
...
function BookCard({ b }) {
  ...
  return (
    <Link className="book card" to={`/books/${b.id}`}>
      <img src={img(b.cover)} />
      <div>
        <span className="badge">{b.availability}</span>
        <h3>{b.title}</h3>
        <p>{b.author}</p>
        <small>
          {b.genre} • {b.condition}
...
function BookDetail() {
  ...
  return (
    <section className="detail">
      <img className="coverBig" src={img(b.cover)} />
      <div>
        <span className="badge">{b.availability}</span>
        <h1>{b.title}</h1>
        <h2>{b.author}</h2>
        ...
        <dl>
          ...
          <dt>Owner</dt>
          <dd>{owner?.name}</dd>
          <dt>Exchange location</dt>
          <dd>{b.location}</dd>
        </dl>
        <div className="actions">
          {user?.id === b.ownerId && (
            <Link className="btn ghost" to={`/books/${b.id}/edit`}>
              Edit listing
            </Link>
          )}
          {user ? (
            ...
            >
              {can ? "Request exchange" : "Request unavailable"}
            </button>
          ) : (
  ...
  return (
    <div className="modal" role="dialog" aria-modal="true">
      ...
      >
        ...
        <p>
          Exchange location: <b>{book.location}</b>
        </p>
        <label className="field">
...
function Dashboard() {
  ...
  let my = state.books.filter((b) => b.ownerId === user.id),
...
function BookForm({ edit = false }) {
  ...
  return (
    <form className="panel form" onSubmit={save}>
      ...
      <img className="preview" src={img(f.cover)} />
      <label className="field">
...
function Requests({ history = false }) {
  const { state, user, requestAction } = useApp();
  const [filter, setFilter] = useState("");
  let closed = ["Completed", "Rejected", "Cancelled"];
